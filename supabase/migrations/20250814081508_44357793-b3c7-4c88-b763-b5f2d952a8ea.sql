...
CREATE TRIGGER client_access_audit_trigger
  AFTER SELECT OR UPDATE OR DELETE ON public.clients
...
-- Create secure view for client data with masking
...
SELECT 
  ...
  mask_sensitive_data(idnp, 3) as idnp,
  mask_sensitive_data(telefon, 4) as telefon,
  mask_sensitive_data(adresa, 10) as adresa,
  created_at,
  updated_at
FROM public.clients
